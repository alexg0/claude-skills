...
-   `scripts.run.<id>.available_in`: `local`, `cloud`, or both. Omit it when the script should appear in local and cloud workspaces.
...
-   When tests mutate a local database, cache, queue, or other stateful service, give each workspace a stable private resource name derived from `CONDUCTOR_WORKSPACE_NAME`, keep the ordinary non-Conductor name as the fallback, and prepare that resource during setup. Follow [Private test resources](references/private-test-resources.md) for the naming algorithm, Rails example, safety rules, and verification checklist.
...
-   When a project has several useful commands, use separate run script IDs such as `dev`, `test`, and `worker`.
-   Use `.conductor/settings.local.toml` for local-only run scripts that should apply to this repository's workspaces on the user's Mac.
-   Use `.conductor/settings.toml` only for shared team run scripts. State clearly that Conductor reflects `settings.toml` changes only after they are merged to the default branch on the remote; changing it on a workspace branch will not update project settings.
...
Project with multiple run scripts:
...
-   New multi-run configurations should use `[scripts.run.<id>]` with `command`; legacy `scripts.run` is still read for a single run script and is migrated to the structured `[scripts.run.<id>]` form when Conductor rewrites the settings file.
...
-   Common run script icons include `play`, `terminal`, `square-terminal`, `server`, `globe`, `database`, `monitor`, `rocket`, `code`, `braces`, `package`, `test-tube`, `bug`, `wrench`, `settings`, `cloud`, `folder`, `file-code`, and `flame`.
